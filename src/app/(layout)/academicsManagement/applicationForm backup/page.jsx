...
const ApplicationForm = ({ config }) => {                               
    ...
    if (config) {
      ...
      Object.entries(config.sections).forEach(([sectionKey, section], index) => {
        if (section.enabled) {
          ...
          // Use fieldsOrder to maintain the correct order of fields
  ...
  const renderSection = (sectionKey, section, index) => {
    ...
    section.fieldsOrder?.forEach(fieldKey => {
      ...
      if (currentRowSpan + colSpan > 2) {
        // Start a new row
        rows.push(currentRow);
        currentRow = [{ fieldKey, field }];
        currentRowSpan = colSpan;
      } else {
        // Add to current row
        currentRow.push({ fieldKey, field });
        currentRowSpan += colSpan;
      }
    });
          ...
          <div className="px-4 md:px-6">
            {rows.map((row, rowIndex) => (
              <div key={rowIndex} className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                {row.map(({ fieldKey, field }) => (
                  <div key={fieldKey} className={`col-span-${field.colSpan || 1}`}>
                    {renderField(fieldKey, field, sectionKey)}
                  </div>
                ))}
              </div>
