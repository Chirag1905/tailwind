...
const CourseCreate = (props) => {
  ...
  const { coursePostData, loading, error } = useSelector((state) => state.course);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      dispatch(postCourseRequest({ data: formData, token }));
    } catch (err) {
  ...
  useEffect(() => {
    if (!coursePostData?.message) return;
    toast.success(coursePostData.message, {
      position: "top-right",
      duration: 4000,
    });
    ...
    dispatch(postCourseSuccess(null));
    closeModal();
  }, [dispatch, coursePostData, closeModal]);
  ...
  useEffect(() => {
    ...
    dispatch(postCourseSuccess(null));
    dispatch(postCourseFailure(null));
  }, [error]);
