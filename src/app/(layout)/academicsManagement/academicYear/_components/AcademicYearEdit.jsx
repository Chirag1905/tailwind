...
const AcademicYearEdit = (props) => {
  ...
  const { academicYearDataFetch, academicYearPutData, loading, error } = useSelector((state) => state.academicYear);
  ...
  useEffect(() => {
    if (selectedItem) {
      dispatch(getAcademicYearFetchRequest({
        token,
        id: selectedItem?.id,
      }));
    }
  ...
  useEffect(() => {
    if (academicYearDataFetch?.data) {
      const { academicYearName, startDate, endDate, isActive } = academicYearDataFetch?.data;
      setFormData({
  ...
  useEffect(() => {
    ...
    closeModal();
  }, [dispatch, academicYearPutData, closeModal, token]);
  ...
  useEffect(() => {
    ...
    dispatch(putAcademicYearSuccess(null));
    dispatch(putAcademicYearFailure(null));
  }, [error]);
