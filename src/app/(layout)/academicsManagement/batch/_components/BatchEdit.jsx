...
const BatchEdit = (props) => {
  ...
  const { batchDataFetch, batchPutData, loading, error } = useSelector((state) => state.batch);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const params = {
        campusBatchName: formData?.campusBatchName,
        isActive: formData?.isActive,
        campusCourseId: courseId,
        academicYearId: selectedAcademicYear,
      }
      dispatch(putBatchRequest({
        data: params,
        token,
        id: selectedItem?.id,
      }));
  ...
  useEffect(() => {
    if (batchDataFetch?.data) {
      const { campusBatchName, isActive } = batchDataFetch?.data;
      setFormData({
        campusBatchName: campusBatchName || "",
  ...
  useEffect(() => {
    if (!batchPutData?.message) return;
    toast.success(batchPutData?.message, {
      position: "top-right",
      duration: 5000,
    });
    ...
    dispatch(putBatchSuccess(null));
    closeModal();
  }, [dispatch, batchPutData, closeModal]);
